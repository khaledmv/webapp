<section id="hero" class="hero d-flex align-items-center">

  <div class="container">
    <div class="row">
      <div class="col-lg-6 d-flex flex-column justify-content-center">
        <h3 data-aos="fade">Help Store Owners grow through </h3>
        <h1 data-aos="fade">Innovation, consistency and quality of delivered <span style="color:#FF498A;">Web Design</span>  services</h1>
        <h2 class="header__iconBox" data-aos="fade" data-aos-delay="200">
          <span class="header-icon"><ion-icon class="header-icon-orange" name="stats-chart-outline"></ion-icon> 99% Store Performance</span>
          <span class="header-icon"><ion-icon class="header-icon-orangered" name="speedometer-outline"></ion-icon> Quality & Fast Delivery</span>
         </h2>
        <div data-aos="fade-up" data-aos-delay="400">
          <div class="text-center text-lg-start">
            <a href="https://calendly.com/shopify-khaled/30min" class="btn-get-started scrollto d-inline-flex align-items-center justify-content-center align-self-center">
              <span>Free 30m. Consultation</span>
              <i class="bi bi-arrow-right"></i>
            </a>
          </div>
        </div>
      </div>
      <div class="col-lg-6 hero-img" data-aos="zoom-out" data-aos-delay="100">
        <img src="assets/img/cms.svg" class="img-fluid hero__svg" alt="webaid digital agency" loading="lazy" >
        
      </div>
    </div>
  </div>

</section>